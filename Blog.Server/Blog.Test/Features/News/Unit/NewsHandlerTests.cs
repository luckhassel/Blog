...
namespace Blog.Test.Features.News.Unit
{
    public class NewsHandlerTests
    {
        ...
        public async Task Create_ShouldNotCreateNews_WhenRequestIsInvalid(string title, string description, int authorId)
        {
            ...
            var newsEntityCreatedPreviously = NewsEntity.Create(title, description, userEntityResponseUserRepositoryNull).Value;
        ...
        public async Task Get_ShouldGetNewsById_WhenRequestIsValid()
        {
            ...
            var authorEntity = await CreateUserPreviouslyAsync(requestUserPreviously, 0);
        ...
        {
            return UserEntity.Create(request.FirstName, request.LastName, request.Email, "hashedPassword").Value;
        }
        ...
        {
            var user = await CreateUserPreviouslyAsync(requestUser, 0);
            var news = NewsEntity.Create(requestNews.Title, requestNews.Description, user).Value;
            return news;
        }
    }
}
